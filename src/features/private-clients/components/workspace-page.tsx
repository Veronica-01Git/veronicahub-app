...
/**
 * O workspace privado de um cliente, atrás do selo (e, quando o registro
 * exige, de conta verificada). Mora fora da rota porque mais de um endereço o
 * renderiza: `/clientes/$clientSlug` para a maioria e `/clientes/lz-team/painel`
 * para o LZ, cujo `/clientes/lz-team` virou página pública. O portão é o mesmo
 * nos dois: `getWorkspaceAccess` no servidor.
 */
export function PrivateClientWorkspace({ clientSlug }: { clientSlug: string }) {
  ...
  return (
    <div className="mt-5">
      <Panel title="Ambiente reservado">
        ...
        <Link
          to="/clientes/lz-team"
          className="mt-5 inline-flex min-h-11 items-center gap-2 rounded-sm border border-neon-green/45 px-4 font-mono-tech text-[10px] uppercase tracking-widest text-neon-green transition hover:bg-neon-green/[.08]"
        >
          Abrir página pública <ArrowRight className="h-4 w-4" aria-hidden />
        </Link>
      </Panel>
    </div>
  );

...
export function calcEngagement(input: EngagementInput): EngagementResult {
  ...
  const viewToFollowerRatio =
    input.followers > 0 && input.avgViews > 0 ? input.avgViews / input.followers : null;
  ...
  if (tier === "baixa") {
    tips.push({
      ...
      detail:
        "Vídeos com engajamento baixo costumam perder o espectador logo no início. Abra com o resultado ou a pergunta, não com introdução.",
    });
    tips.push({
      title: "Poste com mais consistência",
      detail:
        "O algoritmo do TikTok favorece contas ativas. Defina uma frequência realista e mantenha por pelo menos 2-3 semanas antes de reavaliar.",
    });
  }
  ...
  if (commentRatio < 0.15) {
    tips.push({
      ...
      detail:
        "Poucos comentários em relação a curtidas e compartilhamentos. Termine os vídeos com uma pergunta direta ou peça a opinião de quem assiste.",
    });
  }
  ...
  if (shareRatio < 0.1) {
    tips.push({
      ...
      detail:
        "Listas, comparações de antes/depois e tutoriais rápidos tendem a gerar mais compartilhamentos do que vídeos só de entretenimento.",
    });
  }
  ...
  if (viewToFollowerRatio !== null && viewToFollowerRatio < 0.3) {
    tips.push({
      ...
      detail:
        "Isso costuma indicar pouco alcance fora da sua base atual. Teste sons e hashtags em alta e publique nos horários de pico do seu público.",
    });
  }
  ...
  if (tier === "otima" || tier === "excelente") {
    tips.push({
      ...
      detail:
        "Com esse engajamento, o próximo passo é monetizar: comece a testar produtos no TikTok Shop nos vídeos que já performam bem.",
    });
  }
  ...
  tips.push({
    ...
    detail:
      "Priorize os formatos que já têm engajamento comprovado antes de criar conteúdo novo só para vender.",
  });
  tips.push({
    title: "Faça pelo menos 1 live de vendas por semana",
    detail:
      "Lives têm alcance orgânico maior no TikTok Shop e permitem mostrar o produto em uso, tirando dúvidas em tempo real.",
  });
  tips.push({
    title: "Ofereça amostra grátis pra criadores afiliados",
    detail:
      "Criadores menores testando seu produto de graça geram prova social e tráfego qualificado com baixo custo de aquisição.",
  });

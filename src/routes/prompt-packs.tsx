...
import { Check, ShieldCheck, Zap, FileStack, Download, Copy, FileText, MessageCircle, Plus, Minus } from "lucide-react";
...
export const Route = createFileRoute("/prompt-packs")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Documentos com comandos prontos pra Nano Banana Pro, Veo, Midjourney e ElevenLabs. Cada pack vem com o Selo de Originalidade Veronica.",
      },
      { property: "og:title", content: "Prompt Packs — Comandos prontos pra IA real" },
      { property: "og:description", content: "Os comandos exatos. As plataformas certas. Resultado hoje." },
...
const TRUST_ITEMS = [
  ...
  { icon: ShieldCheck, label: "Entrega com Selo de Originalidade" },
  { icon: Zap, label: "Atualizado com as plataformas mais recentes" },
];

const DELIVERABLES = [
  { icon: Download, title: "Documento em PDF", desc: "Arquivo único, organizado por plataforma, pra abrir no celular ou no computador." },
  { icon: Copy, title: "Comandos prontos pra copiar", desc: "Cada prompt já formatado — cola na ferramenta e roda, sem precisar adaptar nada." },
  { icon: FileText, title: "Dicas de uso", desc: "Orientação curta em cada comando: quando usar, o que ajustar, o que evitar." },
  { icon: ShieldCheck, title: "Selo de Originalidade", desc: "Carimbo digital de autenticidade Veronica, com número de série, gravado no documento." },
];

const FAQS = [
  {
    q: "Preciso pagar pelas plataformas de IA?",
    a: "Não necessariamente. A maioria das ferramentas (Nano Banana Pro, Veo, Midjourney, ElevenLabs) tem opção gratuita ou período de teste. O pack não inclui assinatura de nenhuma plataforma — só os comandos pra usar nelas.",
  },
  {
    q: "Os comandos funcionam em qualquer conta?",
    a: "Sim. São prompts de texto puro, sem dependência de plano específico — funcionam tanto na conta gratuita quanto na paga da plataforma indicada em cada comando.",
  },
  {
...
function buildPackWhatsappUrl(pack: PromptPack): string {
  const msg = `Olá! Quero contratar o Prompt Pack "${pack.name}" (${formatBRL(pack.priceCents)}).`;
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 font-mono-tech text-[9px] uppercase tracking-widest ${LEVEL_CLASS[pack.level]}`}>
            {pack.level}
          </span>
          <h3 className="mt-3 font-display text-xl text-foreground" style={{ letterSpacing: "-0.02em", lineHeight: "1.1" }}>
            {pack.name}
          </h3>
        </div>
        <VeronicaSeal
      ...
      <div className="flex flex-wrap gap-1.5">
        {pack.platforms.map((p) => (
          <span key={p} className="rounded-full border border-border/60 px-2.5 py-1 font-mono-tech text-[10px] uppercase tracking-widest text-muted-foreground">
            {p}
          </span>
        ))}
      </div>
      ...
      <div className="mt-auto flex flex-col gap-3 border-t border-border/50 pt-4 sm:flex-row sm:items-center sm:justify-between">
        <span className="font-display text-2xl text-neon-green">{formatBRL(pack.priceCents)}</span>
        <a
          href={buildPackWhatsappUrl(pack)}
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex min-h-[44px] w-full items-center justify-center gap-2 rounded-sm bg-neon-green px-5 py-3 font-mono-tech text-[11px] uppercase tracking-widest text-primary-foreground shadow-glow-green transition duration-200 hover:-translate-y-0.5 hover:brightness-110 sm:w-auto"
        >
  ...
  return (
    <div>
      <button
        ...
        aria-expanded={open}
      >
        <span className="font-display text-[15px] text-foreground sm:text-base" style={{ letterSpacing: "-0.01em" }}>
          {q}
        </span>
        <span
          className={`flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-sm border transition ${
            open ? "border-neon-green bg-neon-green/10 text-neon-green rotate-180" : "border-border/60 text-muted-foreground"
          }`}
        >
          {open ? <Minus className="h-3.5 w-3.5" /> : <Plus className="h-3.5 w-3.5" />}
        </span>
      </button>
      <div className="grid overflow-hidden px-5 transition-all duration-300 ease-out sm:px-6" style={{ gridTemplateRows: open ? "1fr" : "0fr" }}>
    ...
    <div className="min-h-screen overflow-x-hidden bg-background text-foreground">
      ...
      <section className="relative overflow-hidden border-b border-border/40 py-14 md:py-20">
        <div
          ...
          style={{ background: "radial-gradient(circle at 20% 10%, oklch(0.85 0.22 155 / 0.16), transparent 55%)" }}
        />
        <div className="relative mx-auto max-w-3xl px-6 text-center">
          ...
          <h1 className="mx-auto mt-6 max-w-2xl font-display text-3xl sm:text-4xl md:text-5xl" style={{ letterSpacing: "-0.03em", lineHeight: "1.05" }}>
            Os comandos exatos. As plataformas certas. <span className="text-neon-green text-glow-green">Resultado hoje.</span>
          </h1>
          <p className="mx-auto mt-5 max-w-xl text-[15px] leading-[1.6] text-muted-foreground sm:text-base">
            Documentos com prompts prontos pra usar em ferramentas de IA reais — Nano Banana Pro, Veo, Midjourney e
            ElevenLabs — enquanto a Veronica Studio ainda não gera direto por aqui.
          </p>
        </div>
      </section>
      ...
      <section className="border-b border-border/40 bg-surface/40 px-6 py-6">
        <div className="mx-auto flex max-w-6xl flex-col items-start gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-center sm:gap-x-8 sm:gap-y-3">
          {TRUST_ITEMS.map((t) => (
            <div key={t.label} className="flex items-center gap-2 font-mono-tech text-[11px] uppercase tracking-widest text-muted-foreground">
      ...
      <section id="packs" className="mx-auto max-w-6xl px-6 py-14 md:py-20">
        <div className="mb-10 flex flex-col gap-3">
          ...
          <h2 className="font-display text-2xl sm:text-3xl md:text-4xl" style={{ letterSpacing: "-0.03em" }}>
            4 packs, 4 formatos, resultado na hora.
          </h2>
        </div>
      ...
      <section className="border-t border-border/40 bg-surface/40 py-14 md:py-20">
        <div className="mx-auto max-w-5xl px-6">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {DELIVERABLES.map((d) => (
              <div key={d.title} className="flex flex-col gap-3 rounded-sm border border-border/60 bg-background/60 p-5">

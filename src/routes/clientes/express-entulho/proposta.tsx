...
const steps = [
  { date: "13 SET", title: "Arquitetura", text: "Personalidade masculina, núcleo conversacional e desenho definitivo da solução." },
  { date: "14 SET", title: "Demonstração", text: "Reunião com o proprietário e coleta das regras reais da operação." },
  { date: "15 SET", title: "Conhecimento", text: "Base de conhecimento e validação do conector com o sistema atual." },
  { date: "16 SET", title: "Homologação", text: "Simulações, correções e refinamento das respostas e agendamentos." },
  { date: "17 SET", title: "Entrega", text: "Ativação do MVP funcional e início dos 30 dias de assistência." },
];
      ...
      <main>
        <section className="relative isolate overflow-hidden border-b border-border/70 py-16 md:py-24">
          ...
          <div className="relative mx-auto grid max-w-7xl gap-12 px-6 lg:grid-cols-[1fr_22rem] lg:items-center">
            <div>
              <div className="flex items-center gap-3 font-mono-tech text-[10px] uppercase tracking-[.2em] text-neon-green"><span className="h-px w-8 bg-neon-green" />Proposta oficial · registrada em 11.09.2026</div>
              ...
              <div className="mt-5 inline-flex items-center gap-2 rounded-full border border-neon-green/35 bg-neon-green/[.08] px-4 py-2 font-mono-tech text-[10px] uppercase tracking-widest text-neon-green"><BadgeCheck className="h-4 w-4" />Entrada de R$ 750 confirmada em 12.09.2026</div>
            </div>
        ...
        <section className="mx-auto max-w-7xl px-6 py-16 md:py-24">
          ...
          <div className="mt-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{steps.map((step, index) => <article key={step.date} className="relative overflow-hidden rounded-sm border border-border/70 bg-white/65 p-6"><div className="font-mono-tech text-[10px] uppercase tracking-widest text-neon-cyan">{step.date}</div><div className="mt-10 font-display text-2xl">{step.title}</div><p className="mt-3 text-sm leading-relaxed text-muted-foreground">{step.text}</p><span aria-hidden className="absolute right-4 top-3 font-display text-5xl text-neon-green/[.08]">0{index + 1}</span></article>)}</div>
        ...
        <section id="investimento" className="border-y border-border/70 bg-gradient-to-br from-neon-green/[.08] via-white to-neon-cyan/[.08] py-16 md:py-24">
          <div className="mx-auto grid max-w-7xl gap-10 px-6 lg:grid-cols-[1fr_.8fr] lg:items-center">
            ...
            <div className="overflow-hidden rounded-sm border border-border/70 bg-white/75 p-6 shadow-lg backdrop-blur">
              <div className="flex justify-between gap-4 border-b border-border/70 pb-5"><span className="text-muted-foreground">Entrada paga · 12/09/2026</span><strong className="text-neon-green">R$ 750</strong></div>
              <div className="flex justify-between gap-4 border-b border-border/70 py-5"><span className="text-muted-foreground">Saldo na entrega · 17/09/2026</span><strong>R$ 750</strong></div>
